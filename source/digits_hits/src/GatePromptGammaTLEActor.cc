  ...
  for( unsigned int k=0; k<sizeZ; k++) {
    for( unsigned int j=0; j<sizeY; j++) {
      for( unsigned int i=0; i<sizeX; i++) {
       if( j<sizeY && i==15 && k==17) //Chossing the coordinates of the Voxels of Interest, numbers of VoI must be <= 85
...
void GatePromptGammaTLEActor::SaveData()
{
  ...
  double mean[sizeX*sizeY*sizeZ]; //104125 = nb of voxels of the actor
  double sigma[sizeX*sizeY*sizeZ];
